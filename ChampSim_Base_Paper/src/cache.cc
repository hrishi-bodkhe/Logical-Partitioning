...
void CACHE:: printInstrIndices(){
    for(uint32_t i = 0; i < LLC_SET; ++i)
        cout << "For way: " << i << " " << instr_indices[i] << endl;
}

//void mySwap(BLOCK **block, uint32_t set, uint32_t x, uint32_t y){
//    BLOCK temp;
//
//    temp.address = block[set][x].address;
//    temp.data = block[set][x].data;
//    temp.full_addr = block[set][x].full_addr;
//    temp.dirty = block[set][x].dirty;
//    temp.prefetch = block[set][x].prefetch;
//    temp.valid = block[set][x].valid;
//    temp.tag = block[set][x].tag;
//    temp.ip = block[set][x].ip;
//    temp.used = block[set][x].used;
//    temp.lru = block[set][x].lru;
//    temp.confidence = block[set][x].confidence;
//    temp.cpu = block[set][x].cpu;
//    temp.delta = block[set][x].delta;
//    temp.depth = block[set][x].depth;
//    temp.instr_id = block[set][x].instr_id;
//    temp.signature = block[set][x].signature;
//
//    block[set][x].address = block[set][y].address;
//    block[set][x].data = block[set][y].data;
//    block[set][x].full_addr = block[set][y].full_addr;
//    block[set][x].dirty = block[set][y].dirty;
//    block[set][x].prefetch = block[set][y].prefetch;
//    block[set][x].valid = block[set][y].valid;
//    block[set][x].tag = block[set][y].tag;
//    block[set][x].ip = block[set][y].ip;
//    block[set][x].used = block[set][y].used;
//    block[set][x].lru = block[set][y].lru;
//    block[set][x].confidence = block[set][y].confidence;
//    block[set][x].cpu = block[set][y].cpu;
//    block[set][x].delta = block[set][y].delta;
//    block[set][x].depth = block[set][y].depth;
//    block[set][x].instr_id = block[set][y].instr_id;
//    block[set][x].signature = block[set][y].signature;
//
//    block[set][y].address = temp.address;
//    block[set][y].data = temp.data;
//    block[set][y].full_addr = temp.full_addr;
//    block[set][y].dirty = temp.dirty;
//    block[set][y].prefetch = temp.prefetch;
//    block[set][y].valid = temp.valid;
//    block[set][y].tag = temp.tag;
//    block[set][y].ip = temp.ip;
//    block[set][y].used = temp.used;
//    block[set][y].lru = temp.lru;
//    block[set][y].confidence = temp.confidence;
//    block[set][y].cpu = temp.cpu;
//    block[set][y].delta = temp.delta;
//    block[set][y].depth = temp.depth;
//    block[set][y].instr_id = temp.instr_id;
//    block[set][y].signature = temp.signature;
//}

uint32_t CACHE::swap_instr_data(uint32_t set, uint32_t way) {
    uint32_t swap_with, new_instr_index, instr_ways_start = instr_indices[set], diff_frm_new_instr_index;

    uint32_t i = 0;
    uint32_t tempIStart = instr_ways_start;
    while(i < LLC_IWAYS){
        tempIStart %= LLC_WAY;
        swap_with = (tempIStart + LLC_IWAYS) % LLC_WAY;

        std::swap(block[set][tempIStart], block[set][swap_with]);
        tempIStart += 1;
        ++i;
    }

//    for (uint32_t i = instr_ways_start; i < (instr_ways_start + LLC_IWAYS); i++){
//        i = i % LLC_WAY;
//        swap_with = (i + LLC_IWAYS) % LLC_WAY;
////        mySwap(block, set, i, swap_with);
////        cout << "way: " << i << " " << swap_with << " before swap lru: " << block[set][i].lru << " " << block[set][swap_with].lru << endl;  // <--
//
//        std::swap(block[set][i], block[set][swap_with]);
//
////        cout << "way: " << i << " " << swap_with << " after swap lru: " << block[set][i].lru << " " << block[set][swap_with].lru << endl;
//    }

    new_instr_index = (instr_ways_start + LLC_IWAYS) % LLC_WAY;
    instr_indices[set] = new_instr_index;

    // Checking if the data way is in the range of new instruction ways
//    if(way >= new_instr_index && way < (new_instr_index + LLC_IWAYS)) {
//        diff_frm_new_instr_index = way - new_instr_index;
//        way = (instr_ways_start + diff_frm_new_instr_index) % LLC_WAY;
//    }
//    cout << endl;
...
//            way = llc_find_victim(fill_cpu, MSHR.entry[mshr_index].instr_id, set, block[set], MSHR.entry[mshr_index].ip, MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].type);
            // *Able to identify instruction and data blocks
//            cout << "Using l1d or l1i: " << unsigned(MSHR.entry[mshr_index].fill_l1i) << " " << unsigned(MSHR.entry[mshr_index].fill_l1d) << endl;
//            cout << "is Instruction: " << unsigned(MSHR.entry[mshr_index].instruction) << " is Data: " << unsigned(MSHR.entry[mshr_index].is_data) << endl;
//            cout << endl;

            // ? CHANGING HERE
            uint32_t instrStart = instr_indices[set];
            uint32_t instrEnd = (instr_indices[set] + LLC_IWAYS - 1) % LLC_WAY;
            uint32_t dataStart = (instrEnd + 1) % LLC_WAY;
            uint32_t dataEnd = (dataStart + LLC_DWAYS - 1) % LLC_WAY;
            uint32_t lineType = MSHR.entry[mshr_index].fill_l1i ? 0 : 1;
            way = llc_find_victim_new(fill_cpu, MSHR.entry[mshr_index].instr_id, set, block[set], MSHR.entry[mshr_index].ip, MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].type,
                                      instrStart, instrEnd, dataStart, dataEnd, lineType);
        }
        else
...
//                cout << "Using l1d or l1i: " << unsigned(MSHR.entry[mshr_index].fill_l1i) << " " << unsigned(MSHR.entry[mshr_index].fill_l1d) << endl;
//                cout << "is Instruction: " << unsigned(MSHR.entry[mshr_index].instruction) << " is Data: " << unsigned(MSHR.entry[mshr_index].is_data) << endl;
//                cout << "lru value: " << block[set][way].lru << endl;
//                cout << endl;
                ...
                uint32_t dataStart = (instrEnd + 1) % LLC_WAY;
                uint32_t dataEnd = (dataStart + LLC_DWAYS - 1) % LLC_WAY;
                uint32_t lineType = MSHR.entry[mshr_index].is_data ? 1 : 0; // changed here from l1i to instruction
                llc_update_replacement_state_new(fill_cpu, set, way, MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].ip, 0, MSHR.entry[mshr_index].type, 0,
                                                 instrStart, instrEnd, dataStart, dataEnd, lineType);
            }
            else
...
//                    if(MSHR.entry[mshr_index].fill_l1d) writeback_packet.fill_l1d = 1; // Modifications to identify data and instruction
//                    if(MSHR.entry[mshr_index].fill_l1i) writeback_packet.fill_l1i = 1; // Modifications to identify data and instruction
...
//                cout << endl;
                ...
                uint32_t dataStart = (instrEnd + 1) % LLC_WAY;
                uint32_t dataEnd = (dataStart + LLC_DWAYS - 1) % LLC_WAY;
                uint32_t lineType = MSHR.entry[mshr_index].is_data ? 1 : 0;  // * <--

                llc_update_replacement_state_new(fill_cpu, set, way, MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].ip, block[set][way].full_addr, MSHR.entry[mshr_index].type, 0,
                                                 instrStart, instrEnd, dataStart, dataEnd, lineType);
            }
            else
            ...
            if (cache_type == IS_LLC) {
                ...
                set_write_count[set]++;
                way_writes[set][way]++;
                if(lastDetectedCycle[fill_cpu] + threshold <= current_core_cycle[fill_cpu]) {
//                    cout << "Indices Before: " << endl;
//                    cout << "For set: " << set << " " << instr_indices[set] << endl;
//                    cout << "way: " << way << " before swap lru: " << block[set][way].lru << endl;
//                    cout << "set: " << set << " before set instr index: " << instr_indices[set] << endl;
                    lastDetectedCycle[fill_cpu] = current_core_cycle[fill_cpu];
                    swap_instr_data(set, way);
//                    cout << "way: " << way << " after swap lru: " << block[set][way].lru << endl;
//                    cout << "set: " << set << " after set instr index: " << instr_indices[set] << endl;
//                    cout << "Indices After: " << endl;
//                    cout << "For set: " << set << " " << instr_indices[set] << endl;
//                    cout << endl;
                }
            }
...
//                cout << unsigned(WQ.entry[index].fill_l1d) << " " << unsigned(WQ.entry[index].fill_l1i) << endl;

//                cout << "Using l1d or l1i: " << unsigned(WQ.entry[index].fill_l1i) << " " << unsigned(WQ.entry[index].fill_l1d) << endl;
//                cout << "is Instruction: " << unsigned(WQ.entry[index].instruction) << " is Data: " << unsigned(WQ.entry[index].is_data) << endl;
//                cout << "lru value: " << block[set][way].lru << endl;
//                cout << endl;
                ...
                uint32_t dataStart = (instrEnd + 1) % LLC_WAY;
                uint32_t dataEnd = (dataStart + LLC_DWAYS - 1) % LLC_WAY;
                uint32_t lineType = WQ.entry[index].is_data ? 1 : 0;
//                cout << "is Instruction: " << unsigned(WQ.entry[index].instruction) << " is Data: " << unsigned(WQ.entry[index].is_data) << endl;

                llc_update_replacement_state_new(writeback_cpu, set, way, block[set][way].full_addr, WQ.entry[index].ip, 0, WQ.entry[index].type, 1,
                                                 instrStart, instrEnd, dataStart, dataEnd, lineType);
            }
            else
...
//                    way = llc_find_victim(writeback_cpu, WQ.entry[index].instr_id, set, block[set], WQ.entry[index].ip, WQ.entry[index].full_addr, WQ.entry[index].type);
                    ...
                    way = llc_find_victim_new(writeback_cpu, WQ.entry[index].instr_id, set, block[set], WQ.entry[index].ip, WQ.entry[index].full_addr, WQ.entry[index].type,
                                              instrStart, instrEnd, dataStart, dataEnd, lineType);
...
//                        cout << endl;
                        ...
                        llc_update_replacement_state_new(writeback_cpu, set, way, WQ.entry[index].full_addr, WQ.entry[index].ip, block[set][way].full_addr, WQ.entry[index].type, 0,
                                                         instrStart, instrEnd, dataStart, dataEnd, lineType);
                    }
                    else
...
//                        cout << "after fill" << endl;
                        set_write_count[set]++;
                        way_writes[set][way]++;
                        if(lastDetectedCycle[writeback_cpu] + threshold <= current_core_cycle[writeback_cpu]) {
//                            cout << "set " << set << " way: " << way << " before swap address: " << block[set][way].address << " data: " << block[set][way].data << " instr index" << instr_indices[set] << endl;
//                            cout << "set: " << set << " before set instr index: " << instr_indices[set] << endl;
//                            cout << "Indices Before: " << endl;
//                            cout << "For set: " << set << " " << instr_indices[set] << endl;
                            lastDetectedCycle[writeback_cpu] = current_core_cycle[writeback_cpu];
                            swap_instr_data(set, way);
//                            cout << "set " << set << " way: " << way << " after swap address: " << block[set][way].address << " data: " << block[set][way].data << " instr index" << instr_indices[set] << endl;
//                            cout << "set: " << set << " after set instr index: " << instr_indices[set] << endl;
//                            cout << endl;
//                            cout << "Indices After: " << endl;
//                            cout << "For set: " << set << " " << instr_indices[set] << endl;
...
//                    cout << "ITLB" << endl;
//                    cout << unsigned(RQ.entry[index].fill_level) << " " << unsigned(RQ.entry[index].fill_l1d) << " " << unsigned(RQ.entry[index].fill_l1i) << " " << unsigned(RQ.entry[index].instruction_pa) << " " << unsigned(RQ.entry[index].data_pa) << " " << unsigned(RQ.entry[index].pf_metadata) << endl;
                }
                else if (cache_type == IS_DTLB) {
		...
		if (RQ.entry[index].type == LOAD) {
			...
			cpu = 0;
                ...
                if (cache_type == IS_LLC) {
                    ...
                    uint32_t dataStart = (instrEnd + 1) % LLC_WAY;
                    uint32_t dataEnd = (dataStart + LLC_DWAYS - 1) % LLC_WAY;
                    uint32_t lineType = RQ.entry[index].is_data ? 1 : 0; // changed here from l1i to instruction
                    llc_update_replacement_state_new(read_cpu, set, way, block[set][way].full_addr, RQ.entry[index].ip, 0, RQ.entry[index].type, 1,
                                                     instrStart, instrEnd, dataStart, dataEnd, lineType);
        ...
        if ((PQ.entry[PQ.head].event_cycle <= current_core_cycle[prefetch_cpu]) && (PQ.occupancy > 0)) {
            ...
            if (way >= 0) { // prefetch hit
                ...
                if (cache_type == IS_LLC) {
                    ...
                    uint32_t dataStart = (instrEnd + 1) % LLC_WAY;
                    uint32_t dataEnd = (dataStart + LLC_DWAYS - 1) % LLC_WAY;
                    uint32_t lineType = PQ.entry[index].is_data ? 1 : 0; // changed here from l1i to instruction
                    llc_update_replacement_state_new(prefetch_cpu, set, way, PQ.entry[index].full_addr, PQ.entry[index].ip, 0, PQ.entry[index].type, 1,
                                                     instrStart, instrEnd, dataStart, dataEnd, lineType);
