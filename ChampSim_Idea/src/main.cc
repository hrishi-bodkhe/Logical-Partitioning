...
void print_set_write_count(CACHE *cache) {
    for (uint32_t i = 0; i < LLC_SET; i++) {
        cout << "set: " << i << " write count: " << cache->set_write_count[i] << endl;
    }
}

void print_way_write_count(CACHE *cache) {
    for (uint32_t i = 0; i < LLC_SET; i++) {
        cout << "For set: " << i << endl;
        for(uint32_t j = 0; j < LLC_WAY; ++j){
            cout << "For way: " << j << " " << cache->way_writes[i * LLC_WAY + j] << endl;
        }
        cout << endl;
    }
}

double_t calcInterV(CACHE *cache, double_t avgWrites) {
    double_t res = 0;
    for (uint32_t i = 0; i < LLC_SET; i++) {
        res += pow((cache->set_write_count[i] / LLC_WAY - avgWrites), 2);
    }

    res = res / (LLC_SET - 1);
    return ((1 / avgWrites) * pow(res, 0.5));
}

double_t calcAvgWrites(CACHE *cache) {
    double_t avgWrites = 0;
    for (uint32_t i = 0; i < LLC_SET; i++) {
        avgWrites += cache->set_write_count[i];
    }

    avgWrites /= (LLC_SET * LLC_WAY);

    return avgWrites;
}

double calcIntraV(CACHE *cache, double_t avgWrites){
    double intraV = 0;
    ...
    for(uint32_t i = 0; i < LLC_SET; ++i){
        ...
        for(uint32_t j = 0; j < LLC_WAY; ++j){
//            cout << double(cache->way_writes[i * LLC_WAY + j]) - double(cache->set_write_count[i] / LLC_WAY) << endl;
            res += (pow((double(cache->way_writes[i * LLC_WAY + j]) - double(cache->set_write_count[i] / LLC_WAY)), 2)) / (LLC_WAY - 1);
        }
//        cout << res << endl;
        res = pow(res, 0.5);
//        cout << res << endl;
        intraV += res;
//        cout << "intraV: " << intraV << endl;
//        cout << endl;
    }

    intraV /= (LLC_SET * avgWrites);
...
int main(int argc, char** argv)
{
    ...
    cout << "Cache set writes status: " << endl;
    print_set_write_count(&uncore.LLC);
    cout << "Cache way writes status: " << endl;
    print_way_write_count(&uncore.LLC);

    double avgWrites = calcAvgWrites(&uncore.LLC);
    cout << "Average Write counts: " << avgWrites << endl;
    cout << "Intra-Set Write Variation: " << calcIntraV(&uncore.LLC, avgWrites) << endl;
    cout << "Inter-set write variation: " << calcInterV(&uncore.LLC, avgWrites) << endl;
