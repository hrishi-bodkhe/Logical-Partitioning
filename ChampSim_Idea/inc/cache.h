...
class CACHE : public MEMORY {
  public:
    ...
    uint32_t instr_indices[LLC_SET];
    uint64_t set_write_count[LLC_SET];
    uint64_t way_writes[LLC_WAY * LLC_SET];
    uint64_t setDataCounter[LLC_SET];
    uint64_t setInstCounter[LLC_SET];
    ...
    CACHE(string v1, uint32_t v2, int v3, uint32_t v4, uint32_t v5, uint32_t v6, uint32_t v7, uint32_t v8) 
        ...
        for (uint32_t i=0; i<NUM_SET; i++) {
            ...
            if (v1 == "LLC"){
                for (uint32_t j = 0; j < LLC_IWAYS; j++) {
//                    cout << j << endl;
                    block[i][j].lru = j;
                }

                for(uint32_t j = 0; j < LLC_DWAYS; j++) {
//                    cout << j << endl;
                    block[i][LLC_IWAYS + j].lru = j;
                }
//                for(uint32_t j = 0; j < LLC_WAY; ++j)
//                    cout << j << " " << block[i][j].lru << endl;
            }
            else {
	...
	total_miss_latency = 0;
        ...
        memset(instr_indices, 0, sizeof(instr_indices));
        memset(set_write_count, 0, sizeof(set_write_count));
        memset(way_writes, 0, sizeof(way_writes));
    };
    ...
    uint32_t swap_instr_data(uint32_t set, uint32_t way);
    ...
    void add_mshr(PACKET *packet),
         ...
         llc_update_replacement_state_new(uint32_t cpu, uint32_t set, uint32_t way, uint64_t full_addr, uint64_t ip,
                                          uint64_t victim_addr, uint32_t type, uint8_t hit, uint32_t instr_start_way,
                                          uint32_t instr_end_way, uint32_t data_start_way, uint32_t data_end_way,
                                          uint32_t line_type),
    ...
    uint32_t
    lru_victim_new(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, uint64_t ip,
                   uint64_t full_addr,
                   uint32_t type, uint32_t inst_way_start, uint32_t inst_way_end, uint32_t data_way_start,
                   uint32_t data_way_end, uint32_t line_type);

    void
    lru_update_new(uint32_t set, uint32_t way, uint32_t instr_start_way, uint32_t instr_end_way,
                   uint32_t data_start_way,
                   uint32_t data_end_way, uint32_t line_type);

    uint32_t llc_find_victim_new(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, uint64_t ip,
                                 uint64_t full_addr, uint32_t type, uint32_t inst_way_start, uint32_t inst_way_end,
                                 uint32_t data_way_start, uint32_t data_way_end, uint32_t line_type);

    uint32_t swap_instr_data(uint32_t set, uint32_t way, uint32_t num_instr_ways, uint32_t associativity);

    void printInstrIndices();
};
